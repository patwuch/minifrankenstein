...
// function to transform the metadate encoded in teiHeader with the xsl stylesheet "Frankenstein_meta.xsl", this will apply the templates and output the text in the html <div id="stats">
  ...
  var visible_mary = document.getElementsByClassName('#MWS'); // SHOULD BE WITHOUT #?
  ...
  var PercyArray = Array.from(visible_percy);
    ...
    PercyArray.forEach(function(element) {
     ...
     MaryArray.forEach(function(element) {
      element.style.color = 'blue'; // Highlight Mary's text in blue
    });
    PercyArray.forEach(function(element) {
      element.style.color = 'black'; // Show Percy's text in black
    });
    } else {
     ...
     MaryArray.forEach(function(element) {
      element.style.color = 'black'; // Show Mary's text in black
    });
    PercyArray.forEach(function(element) {
      element.style.color = 'red'; // Highlight Percy's text in red
    });
    }
  }
...
function toggleReadingText() {
  ...
  var isReadingText = document.body.classList.toggle('reading-text');

  if (isReadingText) {
    // Remove deletions
    Array.from(deletions).forEach(del => del.style.display = 'none');
    
    // Make additions inline
    Array.from(additions).forEach(add => {
      if (add.classList.contains('supraAdd') || add.classList.contains('infralinear-text')) {
    ...
    // Restore deletions
    Array.from(deletions).forEach(del => del.style.display = 'inline');
    
    // Restore additions to superscript
    Array.from(additions).forEach(add => {
      if (add.classList.contains('supraAdd')) {
        add.style.verticalAlign = 'super';
        add.style.fontSize = 'smaller';
      } else if (add.classList.contains('infralinear-text')) {
        add.style.verticalAlign = 'sub';
        add.style.fontSize = 'smaller';
      }
    });
  }
